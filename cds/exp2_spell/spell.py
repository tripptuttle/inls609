    ...
    for key, value in dictionary.items():
        ...
        for word in value.split():
            if not d.check(word):
                ...
                if suggestions:
                    numInclude = math.ceil(len(suggestions) * .20)
...
dict = get_xml_text('topics2016.xml', 'topic', 'note') #xml file, parent element, and child element you want to process

processedText = tokenize_text(dict)

#print(processedText)

spelled = checkSpell(processedText)
#print(spelled)

write_to_trec_format('spell_notesOnly.xml', processedText)
write_to_trec_format('spell_notesAndSpellSuggest.xml', spelled)
